...
macro StartVariables(loc) {
	global evaluate var_loc({loc})
  global evaluate var_loc_start({loc})
  print "\n-----\nVariable section at 0x"
  hexprint({var_loc})
  print ":\n"
}
macro Variable(name, size) {
	if !{defined var_loc} {
		error "Missing StartVariables()!"
	}
	if {var_loc} < 0 {
		error "Missing StartVariables()!"
	}
	constant {name}({var_loc})
  print "'{name}': 0x"
  hexprint({var_loc})
  print ", size = 0x"
  hexprint({size})
  print "\n"
	global evaluate var_loc({var_loc} + {size})
}
macro EndVariables() {
  evaluate size({var_loc} - {var_loc_start})
  print "Variable section size: ", {size}, "\n"
	global evaluate var_loc(-1)
}
